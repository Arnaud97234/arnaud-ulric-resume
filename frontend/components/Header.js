...
function Header() {
    ...
    useEffect(() => {
        fetch("https://arnaud-ulric-resume-backend.vercel.app/users/arnaud.ulric@gmail.com", {mode: "cors", headers: {"Access-Control-Allow-Origin": "*"}}).then(response => response.json()).then(data => {
            addUser(data.user)
        })
    }, [])
