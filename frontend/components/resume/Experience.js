        ...
        return (
            <div className='box' key={key} id={styles.experienceBox}>
                <div className={styles.expTitle}>
                    <Divider component="div" textAlign='left'><div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', margin: '10px 0px' }}>
                        <FontAwesomeIcon icon={faLocationArrow} style={{ marginRight: '15px' }} />
                        <h4 className={styles.company}>{e.company}</h4>
                        <span className={styles.duration}>{startMonth}/{startYear} - {endMonth}/{endYear} ({duration} years)</span>
                    </div></Divider>
